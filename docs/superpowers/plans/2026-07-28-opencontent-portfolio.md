...
New repo location: `~/workspace/openportfolio` (GitHub `iserlabs/openportfolio`, created in Task A1).
...
**Files:**
- Create: `~/workspace/openportfolio/` — `package.json`, `pnpm-workspace.yaml`, `turbo.json`, `apps/site/` (Next.js 16 app: `package.json`, `next.config.ts`, `tsconfig.json`, `app/layout.tsx`, `app/page.tsx`, `postcss.config.mjs`, `app/globals.css`), `packages/lexicons/` (`package.json`, `tsconfig.json`, `src/index.ts`), `.github/workflows/ci.yml`, `.gitignore`, `README.md`
