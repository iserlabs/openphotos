...
- [ ] **Mobile LCP attribution vs CSS columns** — remaining mobile-score variance (80 vs 89) is Chrome reporting the LCP image's paint ~4s after its bytes arrive (Render Delay phase) while the filmstrip shows the viewport fully painted at ~3s; suspect the CSS-multicol masonry defers/mis-attributes image paint. Candidate fix: replace `columns-*` with a row-major CSS grid masonry (also fixes column-major feed-order quirk). Would likely close the gap to ≥90 (criterion 4)
